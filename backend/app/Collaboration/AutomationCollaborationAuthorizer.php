...
final class AutomationCollaborationAuthorizer implements CollaborationResourceAuthorizer
{
    ...
    {
        if (($user->status !== null && $user->status !== 'active') || (int) $user->organization_id !== (int) $resource->getAttribute('organization_id')) return null;
        return ResourceCollaborator::query()->where([
            'resource_type' => 'automation', 'resource_id' => $resource->getKey(), 'user_id' => $user->id,
        ])->value('permission');
    }
