...
final class AutomationPublicationReviewController extends Controller
{
    public function __construct(private AutomationPublicationService $service, private ReviewClaimService $claims, private ReviewHistoryService $history) {}

    public function index(Request $request)
    {
        return response()->json(ResourcePublicationSubmission::where('resource_type', 'automation')
            ->whereIn('resource_id', Automation::where('organization_id', $request->user()->organization_id)->select('id'))
    ...
    {
        abort_unless($s->resource_type === 'automation'
            && Automation::whereKey($s->resource_id)->where('organization_id', request()->user()->organization_id)->exists(), 404);
    }

    private function data($s): array
    {
        $latest = ResourceRevision::where(['resource_type' => 'automation', 'resource_id' => $s->resource_id])->latest('revision_number')->first();
