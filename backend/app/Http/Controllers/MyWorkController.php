...
{
    public function __invoke(Request $request, MyWorkService $service)
    {
        $allowed = ['resource_type', 'lifecycle', 'relationship', 'q', 'per_page', 'page'];
        if (array_diff(array_keys($request->query()), $allowed)) return response()->json(['message' => 'Unsupported My Work parameter.'], 422);
        $filters = $request->validate([
            ...
            'relationship' => ['nullable', Rule::in(['creator', 'contributor', 'draft'])],
