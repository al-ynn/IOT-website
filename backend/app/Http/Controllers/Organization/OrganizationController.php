...
class OrganizationController extends Controller
{
    ...
    public function permissions(Request $request) { $all=['device.view','device.manage','dashboard.view','dashboard.manage','organization.manage','members.manage','audit.view','automation.view','automation.create','automation.update','automation.delete','automation.execute','automation.manage','analytics.view'];$allowed=array_values(array_filter($all,fn($p)=>$request->user()->hasOrganizationPermission($p)));return array_map(fn($name)=>['id'=>$name,'name'=>$name,'description'=>'','category'=>strstr($name,'.',true)],$allowed); }
