...
final class DashboardWidgetRegistry
{
    ...
    private const DEFINITIONS=[
        ...
        'label'=>['name'=>'Label','description'=>'Safe static or authorized Device value label.','category'=>'Tiles','contexts'=>['personal','device'],'contract'=>'safe_label','default'=>['title'=>'Label'],'layout'=>['w'=>3,'h'=>3,'minW'=>1,'minH'=>2,'maxW'=>6,'maxH'=>6]],
        ...
        'device_connection_map'=>['name'=>'Device connection statuses map','description'=>'Authorized Device online/offline status visualization.','category'=>'Maps','contexts'=>['personal','device'],'contract'=>'authorized_device_status_markers','default'=>['title'=>'Device connection statuses map'],'layout'=>['w'=>4,'h'=>2,'minW'=>2,'minH'=>2,'maxW'=>12,'maxH'=>8]],
        ...
        'metric_by_devices'=>['name'=>'Metric by devices','description'=>'Bounded metric comparison across authorized Devices.','category'=>'Charts','contexts'=>['personal','device'],'contract'=>'authorized_device_series','default'=>['title'=>'Metric by devices','timeRange'=>'24h','chartType'=>'line'],'layout'=>['w'=>4,'h'=>2,'minW'=>2,'minH'=>2,'maxW'=>12,'maxH'=>8]],
        ...
        'events_by_device'=>['name'=>'Events by devices','description'=>'Event counts for authorized Devices only.','category'=>'Events','contexts'=>['personal','device'],'contract'=>'authorized_device_event_counts','default'=>['title'=>'Events by devices'],'layout'=>['w'=>4,'h'=>3,'minW'=>3,'minH'=>2,'maxW'=>12,'maxH'=>12]],
        ...
        'gauge'=>['name'=>'Gauge','description'=>'Numeric telemetry value within explicit display bounds.','category'=>'Telemetry','contexts'=>['personal','device'],'contract'=>'single_numeric_value','default'=>['title'=>'Gauge','timeRange'=>'24h','minimum'=>0,'maximum'=>100],'layout'=>['w'=>3,'h'=>3,'minW'=>2,'minH'=>3,'maxW'=>12,'maxH'=>12]],
        ...
        'status'=>['name'=>'Device Status','description'=>'Factual canonical Device status.','category'=>'Device','contexts'=>['personal','device'],'contract'=>'device_status','default'=>['title'=>'Device Status'],'layout'=>['w'=>2,'h'=>2,'minW'=>2,'minH'=>2,'maxW'=>12,'maxH'=>12]],
    ...
    public function validate(User $user,string $scope,array $widget,?Device $contextDevice=null,?array $authorizedDeviceIds=null):array
    {
        ...
        if($base['type']==='label'){
            $fontSize=max(12,min(64,(int)($widget['configuration']['fontSize']??24)));
            $limits['minW']=max(1,min(4,(int)ceil($fontSize/16)));
            $limits['minH']=max(2,min(3,1+(int)ceil($fontSize/32)));
        }
        ...
        if(str_starts_with($type,'global_')||in_array($type,self::NO_DEVICE,true)){
            $rules=$type==='label'?['staticValue'=>['nullable','string','max:500'],'unit'=>['nullable','string','max:20'],'fontSize'=>['nullable','integer','between:12,64'],'fontStyle'=>['nullable',Rule::in(['normal','medium','bold','italic'])]]:[];
