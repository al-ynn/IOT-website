...
export const test = base.extend({
  isolatedBackend: [async ({}, use, info) => {
    ...
    try { await health(child); await use(undefined); } finally { await stop(child); for (const suffix of ["", "-journal", "-wal", "-shm"]) fs.rmSync(`${db}${suffix}`, { force: true }); if (!(await free())) throw new Error(`backend port ${port} remained in use`); }
  }, { auto: true }],
});
export { expect };
