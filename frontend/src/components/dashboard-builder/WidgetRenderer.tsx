...
export default function WidgetRenderer({ widget, editable = false, onComment, timeRange }: { widget: Widget; editable?: boolean; onComment?:()=>void; timeRange?:"1h"|"6h"|"24h"|"7d"|"30d" }) {
  ...
  else if(widget.type==="label")content=<p className={`break-words ${widget.settings.fontStyle==="medium"?"font-medium":widget.settings.fontStyle==="bold"||!widget.settings.fontStyle?"font-bold":widget.settings.fontStyle==="italic"?"italic":"font-normal"}`} style={{fontSize:`${widget.settings.fontSize??24}px`,lineHeight:1.2}}>{widget.settings.staticValue||"Label"} {source?.unit}</p>;
