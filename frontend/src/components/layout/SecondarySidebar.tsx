  ...
  return <aside aria-label={`${context.label} contextual navigation`} className="relative flex h-full flex-col border-r border-[var(--ds-border-subtle)] bg-[var(--ds-card)]">
    ...
    <div className="relative flex h-14 items-center gap-2 border-b border-[var(--ds-border-subtle)] px-3">
      <div className="min-w-0 flex-1">
        <p className="text-[9px] font-semibold uppercase tracking-[0.2em] text-[var(--ds-chart-3)]">Current area</p>
    ...
    <nav aria-label={`${context.label} contextual pages`} className="relative flex-1 space-y-5 overflow-y-auto p-3">
      {sections.map(section=><section key={section.id} aria-labelledby={`secondary-${context.key}-${section.id}`}>
        ...
        <div className="space-y-0.5">{section.items.map(entry=>{const Icon=entry.icon;const active=activeItemId===entry.id;return <Link key={entry.id} to={entry.path} onClick={onNavigate} aria-current={active?"page":undefined} className={`relative flex min-h-9 items-center gap-2 rounded-[8px] px-2.5 text-xs focus-visible:ring-2 focus-visible:ring-[var(--ds-focus)] ${active?"text-[var(--ds-chart-3)]":"text-[var(--ds-text-muted)] hover:text-[var(--ds-text)]"}`} style={active?{background:"linear-gradient(90deg, color-mix(in oklab, var(--ds-chart-3) 20%, transparent), color-mix(in oklab, var(--ds-chart-3) 4%, transparent))",boxShadow:"inset 0 0 0 1px color-mix(in oklab, var(--ds-chart-3) 42%, transparent)"}:undefined}>
          {active&&<span aria-hidden className="absolute left-0 top-1/2 h-4 w-[3px] -translate-y-1/2 rounded-r-full" style={{background:"linear-gradient(180deg, var(--ds-chart-3), var(--ds-chart-4))",boxShadow:"0 0 10px 1px var(--ds-chart-3)"}}/>}
