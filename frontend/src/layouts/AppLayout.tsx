...
export default function AppLayout({children}:{children?:ReactNode}){
  ...
  return <div className="app-backdrop min-h-screen bg-[var(--ds-bg)] text-[var(--ds-text)]">
    ...
    <div className={`transition-[padding] duration-200 ${padding}`}>
      <AppHeader onOpenNavigation={()=>{setMobileSecondaryOpen(false);setMobilePrimaryOpen(true)}} onOpenContextNavigation={()=>{setMobilePrimaryOpen(false);setMobileSecondaryOpen(true)}} hasContextNavigation={Boolean(secondary)} contextNavigationLabel={secondary?.context.label}/>
