 ...
 return <div className="min-h-screen overflow-x-hidden bg-[var(--ds-bg)] text-[var(--ds-text)]">
  <header className="sticky top-0 z-40 border-b border-[var(--ds-border-subtle)] bg-[color:var(--ds-surface)]/95 backdrop-blur">
   <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
    <Link to="/" className="flex items-center gap-2.5 text-sm font-bold" aria-label="IoT Platform home"><span className="grid h-8 w-8 place-items-center rounded-[8px] bg-[var(--ds-primary-hover)] text-white">I</span>IOT PLATFORM</Link>
    ...
    <div className="hidden items-center gap-2 lg:flex"><Link to="/login"><Button variant="ghost">Login</Button></Link><Link to="/login"><Button>Get Started</Button></Link></div>
   ...
   {open&&<nav id="mobile-navigation" aria-label="Mobile navigation" className="border-t border-[var(--ds-border-subtle)] px-4 py-3 lg:hidden">{publicNavigation.map(item=><NavLink key={item.to} to={item.to} end={item.to==="/"} onClick={()=>setOpen(false)} className={({isActive})=>`${navClass({isActive})} block`}>{item.label}</NavLink>)}<div className="mt-3 grid grid-cols-2 gap-2"><Link to="/login" onClick={()=>setOpen(false)}><Button variant="outline" className="w-full">Login</Button></Link><Link to="/login" onClick={()=>setOpen(false)}><Button className="w-full">Get Started</Button></Link></div></nav>}
  </header>
